...
def preprocessing(path_to_eval_dataset, algorithm):
    ...
    combined_df = features.convert_to_dataframe_testing(
        eval_data, path_to_eval_dataset)
...
def false_alarm_rate(y_pred, y_test, hosts_lists):
    ...
    hosts = {"true positive": [], "false positive": [],
    ...
    for i in range(len(y_test)):
        if y_pred[i] == y_test[i] and y_pred[i] == "bot":  # Detection rate
            ...
            print(colors.Colors.LIGHTCYAN +
                  hosts_lists[i] + " : true positive" + colors.Colors.RESET)

        elif y_pred[i] != y_test[i] and y_pred[i] == "bot":  # False alarm rate
            hosts["false positive"].append(hosts_lists[i])
            suspicious_hosts.append(hosts_lists[i])
            print(colors.Colors.LIGHTRED +
                  hosts_lists[i] + " : false positive" + colors.Colors.RESET)

        elif y_pred[i] == y_test[i] and y_pred[i] == "human":
            hosts["true negative"].append(hosts_lists[i])
            print(colors.Colors.LIGHTYELLOW +
                  hosts_lists[i] + " : true negative" + colors.Colors.RESET)

        elif y_pred[i] != y_test[i] and y_pred[i] == "human":
            hosts["false negative"].append(hosts_lists[i])
            print(colors.Colors.LIGHTPURPLE +
                  hosts_lists[i] + " : false negative" + colors.Colors.RESET)

    print(colors.Colors.RED +
          f"####\nFalse alarm rate and detection rate..." + colors.Colors.RESET)
    print(colors.Colors.RED +
          f"Detection rate : {len(hosts['true positive']) / (len(hosts['true positive']) + len(hosts['false negative'])) * 100} %" + colors.Colors.RESET)
    print(colors.Colors.RED +
          f"False alarm rate : {len(hosts['false positive']) / (len(hosts['false positive']) + len(hosts['true negative'])) * 100} %" + colors.Colors.RESET)
    print(colors.Colors.RED +
          f"False negative rate : {len(hosts['false negative']) / (len(hosts['true positive']) + len(hosts['false negative'])) * 100} %" + colors.Colors.RESET)
    print(colors.Colors.RED +
          f"True negative rate : {len(hosts['true negative']) / (len(hosts['false positive']) + len(hosts['true negative'])) * 100} %" + colors.Colors.RESET)

    print(colors.Colors.RED +
          f"Accuracy : {(len(hosts['true positive']) + len(hosts['true negative'])) / (len(hosts['true positive']) + len(hosts['false positive']) + len(hosts['true negative']) + len(hosts['false negative'])) * 100} %" + colors.Colors.RESET)

    print(colors.Colors.RED + f"####\nTotal host : ",
          len(hosts_lists), colors.Colors.RESET)
    print(colors.Colors.RED + "Number of true positive : ",
          len(hosts["true positive"]), colors.Colors.RESET)
    print(colors.Colors.RED + "Number of false positive : ",
          len(hosts["false positive"]), colors.Colors.RESET)
    print(colors.Colors.RED + "Number of false negative : ",
          len(hosts["false negative"]), colors.Colors.RESET)
    print(colors.Colors.RED + "Number of true negative : ",
          len(hosts["true negative"]), colors.Colors.RESET)
    print(colors.Colors.RED + f"####\n" + colors.Colors.RESET)
...
def determine_threshold(y_pred_proba, hosts_list):
    ...
    for i in range(len(y_pred_proba)):
        if abs(y_pred_proba[i][0] - y_pred_proba[i][1]) < threshold:
            print(colors.Colors.GREY +
                  f"{hosts_list[i]} : {y_pred_proba[i]}" + colors.Colors.RESET)
            human_bot.append(i)
...
def print_classification_report(y_pred, y_test):
    ...
    classification = classification_report(
        y_true=y_test, y_pred=y_pred, target_names=['bot', 'human'], output_dict=True)

    print(colors.Colors.PURPLE +
    ...
    print(
        f"Support: {classification['bot']['support']} \n", colors.Colors.RESET)
    ...
    print(
        f"Support: {classification['human']['support']} \n", colors.Colors.RESET)
    ...
    print(
        f"Support: {classification['weighted avg']['support']}", colors.Colors.RESET)
...
def main_eval(trained_model, eval_dataset, output_path_to_suspicious_hosts):
    ...
    # evaluate the model
    clf, y_pred, y_test = eval_model(
        loaded_clf, eval_dataset, algorithm, output_path_to_suspicious_hosts)
