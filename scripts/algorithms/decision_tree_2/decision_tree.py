...
def encoding_features(combined_df) :
    ...
    combined_df['query_type_encoded'] = label_encoder_query_type.fit_transform(combined_df['query_type'].astype(str))

    label_encoder_domain = LabelEncoder()
    combined_df['domain_encoded'] = label_encoder_domain.fit_transform(combined_df['domain'])
    
    label_encoder_host = LabelEncoder()
    combined_df['host_encoded'] = label_encoder_host.fit_transform(combined_df['host'])
    
    label_encoder_timestamp = LabelEncoder()
    combined_df['timestamp_encoded'] = label_encoder_timestamp.fit_transform(combined_df['timestamp'])
...
def train_decision_tree(combined_df) :
    ...
    X = combined_df[['query_type_encoded', 'domain_encoded', 'host_encoded', 'timestamp_encoded']]
    y = combined_df['label']
...
def testing_eval(path_to_test_tcpdump, path_to_test_tcpdump2,  clf):
    ...
    eval_data1, eval_data2 =  parse_training_data(path_to_test_tcpdump, path_to_test_tcpdump2)
    
    combined_df = convert_to_dataframe(eval_data1, eval_data2)

    combined_df = encoding_features(combined_df)

    X_test = combined_df[['query_type_encoded', 'domain_encoded', 'host_encoded', 'timestamp_encoded']]
...
if __name__ == "__main__":
    ...
    bots_data, webclients_data = parse_training_data(path_to_bots_tcpdump, path_to_webclients_tcpdump)
    ...
    path_to_test_tcpdump = "../../../evaluation_datasets/tcpdumps/eval1_tcpdump.txt"
    path_to_test_tcpdump2 = "../../../evaluation_datasets/tcpdumps/eval2_tcpdump.txt"

    X_test, y_test = testing_eval(path_to_test_tcpdump, path_to_test_tcpdump2, clf)
