...
def aggregate_data(data):
    ...
    Args:
        - data: dictionary containing the request and response data
    ...
    if len(response) == 0:
        ...
        # also combine the query_type : we did this because it would be really hard to map it accordingly
...
def generate_features(all_hosts, data):
    ...
    """
    # Features MISC
    average_of_request_length, average_of_response_length = features_misc.get_average_of_query_length(
        data)
    type_of_requests_queried_by_hosts = features_misc.get_type_of_requests_queried_by_hosts(
        data)
    type_of_responses_received_by_hosts = features_misc.get_type_of_responses_received_by_hosts(
        data)

    # Features TIME
    get_all_timing_for_each_host = features_time.get_timing_for_a_session(data)
    get_average_time_between_requests = features_time.get_average_time_between_requests(
        data)
    frequency_of_repeated_requests_in_a_short_time_frame = features_time.frequency_of_repeated_requests_in_a_short_time_frame(
        data)

    # Features NUMBERS
    average_number_of_dots_in_a_domain = features_numbers.get_average_number_of_dots_in_a_domain(
        data)
    number_of_requests_in_a_session = features_numbers.get_number_of_requests_in_a_session(
        data)
    number_of_unique_domains = features_numbers.get_number_of_unique_domains(
        data)
    ...
    for host in all_hosts:
        ...
        features[host]['type_of_requests_queried_by_hosts'] = tuple(sorted(list(
            (type_of_requests_queried_by_hosts[host]))))  # sort the list to have always the same order
        features[host]['type_of_responses_received_by_hosts'] = tuple(sorted(list(
            (type_of_responses_received_by_hosts[host]))))  # sort the list to have always the same order
...
def convert_features_to_numerical(combined_df):
    ...
    # Features MISC
    combined_df['average_of_request_length'] = combined_df['average_of_request_length'].astype(
        'float64')
    combined_df['average_of_response_length'] = combined_df['average_of_response_length'].astype(
        'float64')
    combined_df['type_of_requests_queried_by_hosts'] = combined_df['type_of_requests_queried_by_hosts'].apply(
        lambda x: '_'.join(x))
    combined_df['type_of_responses_received_by_hosts'] = combined_df['type_of_responses_received_by_hosts'].apply(
        lambda x: '_'.join(x))

    # Features TIME
    combined_df['average_time_for_a_session'] = combined_df['average_time_for_a_session'].astype(
        'float64')
    combined_df['average_time_between_requests'] = combined_df['average_time_between_requests'].astype(
        'float64')
    combined_df['frequency_of_repeated_requests_in_a_short_time_frame'] = combined_df[
        'frequency_of_repeated_requests_in_a_short_time_frame'].astype('float64')

    # Features NUMBERS
    combined_df['average_number_of_dots_in_a_domain'] = combined_df['average_number_of_dots_in_a_domain'].astype(
        'float64')
    combined_df['number_of_requests_in_a_session'] = combined_df['number_of_requests_in_a_session'].astype(
        'float64')
    combined_df['number_of_unique_domains'] = combined_df['number_of_unique_domains'].astype(
        'float64')
    combined_df['average_counts'] = combined_df['average_counts'].astype(
        'float64')
...
def convert_to_dataframe_training(bots_data, webclients_data):
    ...
    combined_df = pd.concat([combined_df, pd.DataFrame(
        webclients_features_df)], ignore_index=True)
...
def read_botlist(path_to_botlist):
    ...
    Args:
        - path_to_botlist: path to the botlist
...
def get_associated_botlist(path_to_eval_dataset):
    ...
    Returns:
        - path to the botlist associated to the evaluation dataset
    """
