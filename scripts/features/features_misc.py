...
def get_average_of_query_length(aggregated_data):
    ...
    for data in aggregated_data:
        ...
        if host in query_length_by_host:
            query_length_by_host[host].append(
                (data['length_request'], data['length_response']))
        else:
            query_length_by_host[host] = [
                (data['length_request'], data['length_response'])]

    average_of_request_length = {key: mean(
        [int(x[0]) for x in value]) for key, value in query_length_by_host.items()}
    average_of_response_length = {key: mean(
        [int(x[1]) for x in value]) for key, value in query_length_by_host.items()}
...
def get_type_of_requests_queried_by_hosts(aggregated_data):
    ...
    for data in aggregated_data:
        ...
        if host in type_of_requests_queried_by_hosts:
            type_of_requests_queried_by_hosts[host].update(
                set(data['request_type']))
        else:
...
def get_type_of_responses_received_by_hosts(aggregated_data):
    ...
    for data in aggregated_data:
        ...
        if host in type_of_responses_received_by_hosts:
            type_of_responses_received_by_hosts[host].update(
                set(data['responses'].keys()))
        else:
            type_of_responses_received_by_hosts[host] = set(
                data['responses'].keys())

    return type_of_responses_received_by_hosts


# WILL NOT BE USED DIRECTLY AS A FEATURE
def get_all_requests_and_responses_for_an_host(aggregated_data):
    """
