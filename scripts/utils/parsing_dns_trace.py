...
def extract_dns_query_type(line):
    ...
    pattern_to_find = r'\b(?:' + '|'.join(re.escape(word)
                                          for word in possible_query_types) + r')\b'
...
def extract_DNS_response(line):
    ...
    if answers == 0:  # will always be (0, 1, 0)
        if query_type == None:
            results[constants.NOT_A_RESSOURCE_RECORD] = [
                constants.AUTHORITATIVE_SERVER_RESPONSE]
...
def parsing_file(lines):
    ...
    for line in lines:
        ...
        if trace_id not in paired_data:
            # Initialize dictionary for the ID
            paired_data[trace_id] = {"request": {}, "response": {}}

        elif paired_data[trace_id]["request"] != {} and paired_data[trace_id]["response"] != {}:
            timestamp_req = parse_timestamp(
                paired_data[trace_id]["request"]["timestamp"])
            timestamp_resp = parse_timestamp(
                paired_data[trace_id]["response"]["timestamp"])
...
def parse_training_data(path_to_bots_tcpdump, path_to_webclients_tcpdump):
    ...
    print(colors.Colors.GREEN +
          "Parsing TRAINING datasets completed!\n####" + colors.Colors.RESET)
...
def parse_eval_data(path_to_eval_tcpdump):
    ...
    print(colors.Colors.GREEN +
          "Parsing EVALUATION dataset completed!\n####\n" + colors.Colors.RESET)
    return eval_data


if __name__ == "__main__":
    bots_data, webclients_data = parse_training_data(
        "../../training_datasets/tcpdumps/bots_tcpdump.txt", "../../training_datasets/tcpdumps/webclients_tcpdump.txt")
