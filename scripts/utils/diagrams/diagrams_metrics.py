...
def diagram_features_TP_TN_FP_FN(eval_number):
    """
    Generates a diagram that shows the classification outcomes (TP, TN, FP, FN) for each set of features used in the evaluation.

    Args:
        - eval_number: evaluation number. It can be either "eval1" or "eval2".

    """
    feature_combination = ["All features", "Miscellaneous", "Time", "Numbers",
    ...
    plt.savefig(
        f"../../../diagrams/metrics/features_TP_TN_FP_FN/diagram_features_TP_TN_FP_FN_{eval_number}.png")


def diagram_bayesian_detection_rate(eval_number):
    """
    Made with logistic regression. Calculated with the following formula :

    P(D|B) : Probability of detection given that it is a true positive
    bayesian detection rate
    P(B|D) : Probability that it is a true positive given that it is detected
    P(D) : Probability of detection
    P(B) : Probability that it is a true positive
    ...
    if eval_number == "eval1":
        ...
        false_alarm_rates = [11.11, 24.07, 6.48,
                             4.63, 2.77, 13.88, 10.18, 24.07]
    ...
    for i in range(len(bayesian_detection_rates)):
        bayesian_detection_rates[i] = ((detection_rate[i] * p_bot) / (
            (detection_rate[i] * p_bot) + (false_alarm_rates[i] * (1 - p_bot)))) * 100
    ...
    plt.savefig(
        f"../../../diagrams/metrics/bayesian_detection_rate/diagram_features_bayesian_rate_{eval_number}.png")


def diagram_accuracy_false_alarm_rate(eval_number):
    """
    Made with logistic regression. Creates a bar chart showing the accuracy, false alarm rate and detection rate for different feature combinations.

    Args:
        - eval_number: The evaluation number (either "eval1" or "eval2").
    """
    feature_combination = ["All features", "Miscellaneous", "Time", "Numbers",
                           "Combination 1", "Combination 2", "Combination 3", "Combination 4"]
    ...
    bar1 = ax.bar(indices, accuracy_rates, bar_width,
                  label='Accuracy', color='blue')
    bar2 = ax.bar(indices + bar_width, false_alarm_rates,
                  bar_width, label='False Alarm', color='red')
    bar3 = ax.bar(indices + bar_width*2, detection_rate,
                  bar_width, label='Detection Rate', color='green')
    ...
    plt.savefig(
        f"../../../diagrams/metrics/accuracy_false_alarm_rate/diagram_features_accuracy_false_alarm_rate_{eval_number}.png")


def main_diagrams_metrics():
    """
    This function generates several diagrams related to the evaluation of a model's performance.
    """
